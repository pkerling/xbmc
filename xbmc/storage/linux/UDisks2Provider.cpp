...
namespace
{
const std::string UDISKS2_DESTINATION = "org.freedesktop.UDisks2";
const std::string UDISKS2_MANAGER_PATH = "/org/freedesktop/UDisks2/Manager";
const std::string UDISKS2_MANAGER_INTERFACE = "org.freedesktop.UDisks2.Manager";
}


CUDisks2Device::CUDisks2Device(const char *DeviceKitUDI):
...
void CUDisks2Device::Update()
{
  CVariant properties = CDBusUtil::GetAll("org.freedesktop.UDisks2", m_DeviceKitUDI.c_str(), "org.freedesktop.UDisks2.Device");
  ...
  if (m_isPartition)
  {
    CVariant isRemovable = CDBusUtil::GetVariant("org.freedesktop.UDisks2", properties["PartitionSlave"].asString().c_str(), "org.freedesktop.UDisks2.Device", "DeviceIsRemovable");
...
bool CUDisks2Device::Mount()
{
  if (!m_isMounted && !m_isSystemInternal && m_isFileSystem)
  {
    CLog::Log(LOGDEBUG, "UDisks: Mounting %s", m_DeviceKitUDI.c_str());
    CDBusMessage message("org.freedesktop.UDisks2", m_DeviceKitUDI.c_str(), "org.freedesktop.UDisks2.Device", "FilesystemMount");
...
bool CUDisks2Device::UnMount()
{
  if (m_isMounted && !m_isSystemInternal && m_isFileSystem)
  {
    CDBusMessage message("org.freedesktop.UDisks2", m_DeviceKitUDI.c_str(), "org.freedesktop.UDisks2.Device", "FilesystemUnmount");
...
CMediaSource CUDisks2Device::ToMediaShare()
{
  CMediaSource source;
...
bool CUDisks2Device::IsApproved()
{
...
std::string CUDisks2Device::toString()
{
...
CUDisks2Provider::CUDisks2Provider()
{
  ...
  CDBusError error;
  dbus_bus_add_match(m_connection, "type='signal',interface='org.freedesktop.UDisks2'", error);
  dbus_connection_flush(m_connection);

  if (error)
  {
    error.Log("UDisk2s: Failed to attach to signal");
...
void CUDisks2Provider::Initialize()
{
  ...
  CLog::Log(LOGDEBUG, "UDisks2: Version %s", CDBusUtil::GetVariant(UDISKS2_DESTINATION, UDISKS2_MANAGER_PATH, UDISKS2_MANAGER_INTERFACE, "Version").asString("unknown").c_str());
...
bool CUDisks2Provider::Eject(const std::string& mountpath)
{
  ...
  for (itr = m_AvailableDevices.begin(); itr != m_AvailableDevices.end(); ++itr)
  {
    CUDisks2Device *device = itr->second;
...
bool CUDisks2Provider::PumpDriveChangeEvents(IStorageEventsCallback *callback)
{
  ...
  if (m_connection)
  {
    ...
    if (msg)
    {
      ...
      {
        result = true;
        if (dbus_message_is_signal(msg.get(), "org.freedesktop.UDisks2", "DeviceAdded"))
          DeviceAdded(object, callback);
        else if (dbus_message_is_signal(msg.get(), "org.freedesktop.UDisks2", "DeviceRemoved"))
          DeviceRemoved(object, callback);
        else if (dbus_message_is_signal(msg.get(), "org.freedesktop.UDisks2", "DeviceChanged"))
...
bool CUDisks2Provider::HasUDisks()
{
  return CDBusUtil::TryMethodCall(DBUS_BUS_SYSTEM, "org.freedesktop.UDisks2", "/org/freedesktop/UDisks", "org.freedesktop.UDisks2", "EnumerateDevices");
}

void CUDisks2Provider::DeviceAdded(const char *object, IStorageEventsCallback *callback)
{
  ...
  CUDisks2Device *device = NULL;
    device = new CUDisks2Device(object);
...
void CUDisks2Provider::DeviceRemoved(const char *object, IStorageEventsCallback *callback)
{
  if (g_advancedSettings.CanLogComponent(LOGDBUS))
    CLog::Log(LOGDEBUG, "UDisks: DeviceRemoved (%s)", object);

  CUDisks2Device *device = m_AvailableDevices[object];
  if (device)
  {
...
void CUDisks2Provider::DeviceChanged(const char *object, IStorageEventsCallback *callback)
{
  if (g_advancedSettings.CanLogComponent(LOGDBUS))
...
std::vector<std::string> CUDisks2Provider::EnumerateDisks()
{
  std::vector<std::string> devices;
  CDBusMessage message("org.freedesktop.UDisks2", "/org/freedesktop/UDisks", "org.freedesktop.UDisks2", "EnumerateDevices");
  DBusMessage *reply = message.SendSystem();
  if (reply)
  {
    char** disks  = NULL;
